...
class TestFallbackClusterProviderPagination(unittest.TestCase):
    def test_first_fetch_uses_offset_zero(self) -> None:
        repo = MagicMock()
        repo.query_recent_clusters_sync.return_value = [
        ...
        repo.query_recent_clusters_sync.assert_called_once_with(limit=5, offset=0)

    def test_successive_fetches_advance_offset(self) -> None:
        repo = MagicMock()
        repo.query_recent_clusters_sync.side_effect = [
            [_make_cluster(i) for i in range(5)],
        ...
        self.assertEqual(
            repo.query_recent_clusters_sync.call_args_list[0].kwargs,
            {"limit": 5, "offset": 0},
        )
        self.assertEqual(
            repo.query_recent_clusters_sync.call_args_list[1].kwargs,
            {"limit": 5, "offset": 5},
        )
    ...
    def test_next_fetch_after_wrap_starts_from_post_wrap_offset(self) -> None:
        ...
        self.assertEqual(
            repo.query_recent_clusters_sync.call_args_list[2].kwargs,
            {"limit": 2, "offset": 3},
        )
    ...
    def test_zero_count_returns_empty_without_query(self) -> None:
        ...
        repo.query_recent_clusters_sync.assert_not_called()

    def test_repository_exception_returns_empty(self) -> None:
        repo = MagicMock()
        repo.query_recent_clusters_sync.side_effect = RuntimeError("zmq dead")
